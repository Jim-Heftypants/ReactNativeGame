...
function getneighbors(matrix, i, j, size = 1) {
    ...
    if (i > 0 && j > 0) neighbors.topLeft = (getCenter(i - 1, j - 1, size));
    if (j > 0) neighbors.top = (getCenter(i, j - 1, size));
    if (matrix.length - 1 > i) neighbors.right = (getCenter(i + 1, j, size));
    if (matrix.length - 1 > i && matrix[0].length - 1 > j) neighbors.bottomRight = (getCenter(i + 1, j + 1, size));
    if (matrix[0].length - 1 > j) neighbors.bottom = (getCenter(i, j + 1, size));
    if (matrix.length - 1 > i && j > 0) neighbors.topRight = (getCenter(i + 1, j - 1, size));
    if (matrix[0].length - 1 > j && i > 0) neighbors.bottomLeft = (getCenter(i - 1, j + 1, size));
    return neighbors;
}
